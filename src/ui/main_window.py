...
界面组件基于 PyQt-Fluent-Widgets（PyQt6 分支，源码在 ``reference/PyQt-Fluent-Widgets-PyQt6``）：

- 文件列表：:class:`DropTableWidget`（Fluent ``TableWidget`` + 外部文件拖放）
- 控制面板：Fluent ``ComboBox`` / ``LineEdit`` / ``SwitchButton`` / ``CardWidget``
- 批处理：:class:`ProcessingThread` 后台逐文件调用 :class:`Processor`，
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.mode_combo.addItems(
            [self._t("mode_movie"), self._t("mode_tv"), self._t("mode_music")])
        self.mode_combo.setMinimumWidth(120)
        self.mode_combo.currentIndexChanged.connect(self._on_mode_changed)

        self.btn_add_files = PrimaryPushButton(self)
        self.btn_add_folder = PushButton(self)
        self.btn_remove = PushButton(self)
        self.btn_clear = PushButton(self)
        ...
        self.lbl_lang = StrongBodyLabel(self._t("language_label"))
